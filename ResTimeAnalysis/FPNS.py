...
def worst_case_compute(index_list, execution_list, period_list, priority_list):
    ...
    for i in range(len(index_list)):
        ...
        # Take the maximum of all? (consider the same priority or not)
        maxC = 0
        if i != len(index_list)-1:
            maxind = -1
            for j in range(len(index_list)):
                ind = index_list[j]
                if i != j and priority_list[j] < priority_list[i]:
                    if ind not in stream_dictionary[curr_index]:
                        maxind = ind if execution_list[j] > maxC else maxind
                        maxC = execution_list[j] if execution_list[j] > maxC else maxC
            stream_dictionary[curr_index].add(maxind)

        # Start adding streams to the summation term

        for j in range(len(index_list)):
            # Only lower priority 
            if i != j and index_list[j] not in stream_dictionary[curr_index] and priority_list[j] >= priority_list[i]:
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.csv') and filename[0] not in s:
            ...
            with open(file_path, 'r') as file:
                ...
                priority_list = streams[:,3]
                result_list = worst_case_compute(index_list, execution_list, period_list, priority_list)
...
if __name__ == "__main__":
    for count in range(72):
        stream_dictionary = { (i+1):set() for i in range(13)}
        folder_path = f"Permutations/Folder_{count+1}"
    ...
    print([value for (key, value) in sorted_dict])
    plt.plot([key for (key, value) in sorted_dict], [value for (key, value) in sorted_dict], "*-")
