...
# case 6 : VL6
# ES4 -> (VL13L) -> Switch B -> (VL2 + VL3+ VL9L) -> Switch A -> (VL5 + VL12L) -> ES1

# generate an new system
s = model.System('compare')

# set up all the resouces, or vertexes
source_fromES4 = s.bind_resource(model.Resource("source_fromES4", schedulers.SPNPScheduler()))
...
t1 = source_fromES4.bind_task(model.Task("T1", wcet=8, bcet=1, scheduling_parameter=2))
t2 = first.bind_task(model.Task("VL13L", wcet=30, bcet=1, scheduling_parameter=2))
t3 = switchB_to_A.bind_task(model.Task("T3", wcet=8, bcet=1, scheduling_parameter=2))
t4 = second.bind_task(model.Task("VL2 + VL3+ VL9L", wcet=159, bcet=1, scheduling_parameter=2))
t5 = switchA_to_ES1.bind_task(model.Task("T5", wcet=8, bcet=1, scheduling_parameter=2))
t6 = third.bind_task(model.Task("VL5 + VL12L", wcet=22, bcet=1, scheduling_parameter=2))
